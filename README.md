...
`douyin-global-video-add` 是面向 Codex 的抖音本地生活投放自动化 Skill。向直播全域或门店全域计划添加视频，默认预演，并锁定计划标识及门店 POI 集合。
...
将本仓库目录复制到 Codex 的 Skill 目录，目录名保持为 `douyin-global-video-add`。参数和执行边界见 [SKILL.md](SKILL.md)。
